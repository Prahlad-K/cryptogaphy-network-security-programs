/* C167227 CNS LAB CYCLE -1 ASSIGNMENT - AES*/
#include <iostream> 
#include <bits/stdc++.h>
...
void subBytes_decrypt(unsigned char * state) {
	for (int i = 0; i < 16; i++) {
		state[i] = inv_s[state[i]];
	}
}
void shiftRows_decrypt(unsigned char * state) {
	unsigned char tmp[16];
...
void mixColumns_decrypt(unsigned char * state) {
	unsigned char tmp[16];
...
void round(unsigned char* state, unsigned char* roundKey, int roundNo)
{
	subBytes(state);
	shiftRows(state);
	if(roundNo!=10)
...
void round_decrypt(unsigned char* state, unsigned char* roundKey, int roundNo)
{
	addRoundKey(state, roundKey);
	if(roundNo!=10)
		mixColumns_decrypt(state);
	shiftRows_decrypt(state);
	subBytes_decrypt(state);
}


void encrypt(unsigned char* plaintext, unsigned char* roundKeys, unsigned char* ciphertext)
{
	int roundNo =0;
	...
	while(roundNo<=10)
	{
		if(roundNo==0)
			...
			round(state, roundKeys + 16*roundNo, roundNo);
		roundNo++;
	}
	...
	cout<<"Ciphertext: \n";
	for(int i=0;i<16;i++)
		cout<<setw(2)<<hex<<static_cast<unsigned int>(ciphertext[i])<<" ";
	cout<<endl;
}

void decrypt(unsigned char* ciphertext, unsigned char* roundKeys, unsigned char* decrypted_ciphertext)
{
	int roundNo =10;
	unsigned char state[16];
	...
	while(roundNo>=0)
	{
		if(roundNo==0)
			...
			round_decrypt(state, roundKeys + 16*roundNo, roundNo);
		roundNo--;
	}

	for(int i=0;i<16;i++)
		decrypted_ciphertext[i] = state[i];

	cout<<"Decrypted ciphertext: \n";
	for(int i=0;i<16;i++)
		cout<<setw(2)<<hex<<static_cast<unsigned int>(decrypted_ciphertext[i])<<" ";
	cout<<endl;
...
int main()
{
	...
	unsigned char plaintext[16], ciphertext[16], decrypted_ciphertext[16];
	cout<<"Enter plaintext (in decimal bytes):\n";
	...
	unsigned char key[16] = {
		0x01, 0x04, 0x02, 0x03, 0x01, 0x03, 0x04, 0x0A, 0x09, 0x0B, 0x07, 0x0F, 0x0F, 0x06, 0x03, 0x00
	};
	...
	cout<<"Plaintext: \n";
	for(int i=0;i<16;i++)
		cout<<setw(2)<<hex<<static_cast<unsigned int>(plaintext[i])<<" ";
	cout<<endl;	

	keyExpansion(key, roundKeys);
	encrypt(plaintext, roundKeys, ciphertext);
	decrypt(ciphertext, roundKeys, decrypted_ciphertext);
